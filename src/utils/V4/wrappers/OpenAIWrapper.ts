...
import { Trace, TraceManager } from '../core/TraceManager';
import { LLMInputs, Message, ModelParams, ResponseFormat } from '../../../types';
import { StreamHandler } from '../core/StreamHandler';
import { OpenAIMessageConverter } from '../../V3/message-converters';
import { MODEL_COST_MAPPING } from '../utils/constants';
        ...
        return result.then(
          async (value) => {
            if (this.isStreamingEnabled(args)) {
              const streamHandler = new StreamHandler(trace);
 ...
 */
export function patchOpenAI3(openai: OpenAI): void {
